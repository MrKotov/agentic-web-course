...
This is a monorepo. The four deliverables live side by side so specs, shared fixtures and
the autograder contract stay in one history.

```
apps/platform/        Django + HTMX + Postgres   spec: handover/02
apps/site/            Astro + Starlight          spec: handover/01
apps/examples/        exercise templates         spec: handover/03
packages/autograder/  conformance harness        spec: handover/04
handover/             the specs themselves
docs/                 operational notes and decisions taken while building
```

`apps/site/` and `apps/examples/` publish to separate public repos; students never see
this monorepo. `git subtree` split, documented in `docs/splitting-repos.md`.

## Toolchain

Node 24 (`.nvmrc`), Python 3.13 (`uv`). The root `Makefile` is the entry point for every
common task — prefer adding a target over documenting a long command.

Local development uses SQLite so the platform runs with no services; `DATABASE_URL`
selects Postgres, which is what staging and production use. Anything relying on
Postgres-specific behaviour must be covered by a test that runs against Postgres in CI.
